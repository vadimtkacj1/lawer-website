...
const services = [
  { 
    ...
    description: `זהו השירות המבוקש ביותר שלנו ב"אבי - הבית למשכנתאות". אם המינוס לוחץ וההחזרים החודשיים על הלוואות שונות (רכב, אשראי, הלוואות לכל מטרה) מכבידים עליכם – יש פתרון. אנחנו מתמחים במינוף הנכס הקיים כדי לאחד את כל ההלוואות הקטנות והיקרות למשכנתא אחת מסודרת, בפריסה ארוכה ובריבית נמוכה משמעותית. המטרה: להקטין את ההחזר החודשי באלפי שקלים ולהעניק לכם שקט נפשי.`, 
    Art: LegalStabilityArt 
  },
  { 
    title: "מיחזור משכנתא – למה שתשלמו יותר?", 
    description: `לקחתם משכנתא לפני מספר שנים? ייתכן שהריביות השתנו או שהסטטוס הכלכלי שלכם השתפר. בדיקת כדאיות למיחזור (שאנו מבצעים ללא עלות!) יכולה לחסוך לכם הון לאורך חיי המשכנתא.`, 
    Art: BalanceArt 
  },
  { 
    title: "משכנתא לזוגות צעירים ולציבור החרדי", 
    description: `אנו מתמחים בהשגת אחוזי מימון מקסימליים (כולל פתרונות יצירתיים למימון גבוה במקרים המתאימים), ומכירים לעומק את צרכי הציבור החרדי, הן בחולון והמרכז והן בערים כמו נתיבות. הליווי כולל טיפול בתוכניות "מחיר למשתכן" וסיוע בקבלת משכנתא ראשונה בתנאים אופטימליים.`, 
    Art: TreeHouseArt 
  },
  { 
    title: "פתרונות למסורבי בנקים (BDI שלילי)", 
    description: `קיבלתם סירוב מהבנק? זה לא סוף העולם. בזכות ההיכרות המעמיקה עם המערכת, אני יודעים איך לבנות את התיק מחדש ולפנות לגורמי המימון הנכונים כדי להפוך את ה"לא" ל"כן".`, 
    Art: OpportunityArt 
  },
];

/**
 * Sub-component for individual service items
 */
const ServiceCard = ({ service, index, scrollYProgress }: { service: typeof services[0], index: number, scrollYProgress: MotionValue<number> }) => {
